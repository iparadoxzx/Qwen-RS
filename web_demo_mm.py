...
def _load_model_processor(args):
    ...
    # 从Hugging Face加载LoRA模型
    adapter_path = "iParadoxzx/Qwen-2.5-VL-LoRA-RS"  
...
def _launch_demo(args, model, processor):
    ...
    def add_file(history, task_history, file):
        ...
        if file.name.lower().endswith('.tif'):
            ...
            display_path = temp_file.name
        else:
            display_path = file.name
        
        history = history + [((display_path,), None)]
        task_history = task_history + [((file.name,), None)]  # 保持原始路径用于处理
        return history, task_history
...
<p align="center"><img src="https://modelscope.oss-cn-beijing.aliyuncs.com/resource/qwen.png" style="height: 80px"/><p>"""
        ...
        gr.Markdown("""<center><font size=8>Qwen-RS</center>""")
        gr.Markdown("""\
<center><font size=3>This WebUI is based on Qwen2.5-VL, developed by Alibaba Cloud.</center>""")
        gr.Markdown("""<center><font size=3>本WebUI基于Qwen-RS。</center>""")

        chatbot = gr.Chatbot(label='Qwen-RS', elem_classes='control-height', height=500)
